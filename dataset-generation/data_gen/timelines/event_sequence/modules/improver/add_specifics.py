...
class AddSpecificDetailsToOutlineModule(ParsableBaseModule):
   ...
   and updating the causal relations accordingly.
   """
   ...
   def _preprocess_values(self, values) -> Dict:
      ...
      log(values)
      # Pass through existing causal graph if present
      values['CAUSAL_XML'] = values.get('CAUSAL_XML', '')
      return values
   ...
   def _get_parsers(self) -> List[BasicNestedXMLParser]:
      return [
         BasicNestedXMLParser(
               ...
               is_object=False, allow_empty_list=False,
               result_node='storyitems', remove_node='scratchpad'
         ),
         BasicNestedXMLParser(
               'causal_edges', './/causal/edge',
               is_object=True, allow_empty_list=True,
...
4. Review the date, outline, and existing causal relations of the event:

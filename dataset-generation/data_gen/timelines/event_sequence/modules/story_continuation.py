...
from data_gen.timelines.event_sequence.elements.event import Continuation
from data_gen.util.entity_util import get_entity_categories
from data_gen.util.story_tools import create_history_xml
...
class StoryContinuationModule(ParsableBaseModule):
    ...
    def _get_parsers(self) -> List[BasicNestedXMLParser]:
        return [
            BasicNestedXMLParser('continuations', './/summary', is_object=True, allow_empty_list=False, remove_node='thought_process'),
        ]
...
CAUSAL_CONTINUATION_PROMPT = """
When generating the next event, explicitly link it causally to previous ones (e.g., 'This was triggered by [prior event]', 'As a result of [prior outcome]...').
Add a 'causal_predecessor' field to the event JSON: {"event_id": "prior_event_id"}.
"""
...
def get_instructions(version: str) -> str:
    ...
    return (out + CAUSAL_CONTINUATION_PROMPT).strip()
...
def get_full_outline(summary, base_directory: str):
    ...
    if len(children) == 0:
        ...
        exported_path: str = join(directory, f'{children[0]}/EXPORT_it-5.json')
        if exists(join(directory, f'{children[0]}/EXPORT_it-5.json')):
...
def run(batch_name: str, llm_name: str):
    ...
    for i, summary in enumerate(summaries):
        try:
            ...
            storyline_path: str = join(storyline_directory, 'EXPORT_it-5.json')
...
def main(args):
    ...
    # run(batch_name, "vllm/Qwen/Qwen3-4B-Instruct-2507")
    # run(batch_name, "openrouter/openai/gpt-oss-safeguard-20b")
    # run(batch_name, "openrouter/openai/gpt-4o-2024-08-06")
    # run(batch_name, "openrouter/deepcogito/cogito-v2-preview-llama-70b")
    # run(batch_name, "openrouter/microsoft/phi-4-reasoning-plus")
    # run(batch_name, "openrouter/openai/gpt-5.1")
    # run(batch_name, "openrouter/openai/gpt-5-mini")
    # run(batch_name, "openrouter/qwen/qwen3-30b-a3b-instruct-2507")
    run(batch_name, "openrouter/openai/gpt-oss-120b")
    # run(batch_name, "openrouter/microsoft/phi-3-medium-128k-instruct")

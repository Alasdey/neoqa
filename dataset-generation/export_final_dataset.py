    ...
    for q in questions:
        if q['category'] in {'false-premise', 'uncertain-specificity'}:
            if q['parent_question_id'] is None:
                print("skip parented but incorect question")
                continue
...
def question_has_sufficient_evidence(question, articles):
    all_evidence = set()
    # WTF is that, how did it come to that ?
...
def create_benchmarktl(dataset_name: str):
    ...
    for split in ['test', 'dev']:
        ...
        for question in tqdm(answerable_questions):
            ...
            print(len(all_available_news_for_question) / (question['created_at'] + 1))
...
def normalize_questions(questions):
    ...
    for question in questions:
        ...
        if question['category'] == 'time-span':
            ...
            if norm_question['parent_question_id'] is None:
                continue
